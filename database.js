...
let BankDb = {
    ...
    API: {
        ...
        getAccountByClientId: function (Client) {
            let accArr = []
            for (let i = 0; i < BankDb.Accounts.length; i++) {
                if (BankDb.Accounts[i].ClientId == Client) {
                    accArr.push(BankDb.Accounts[i]);
                }
            }
            return accArr;
        },
...
(function initDB() {
    for (let c = 3; c < 8; c++) {
        ...
        let letter2 = String.fromCharCode(rnd(25) + 65);
        let letter3 = String.fromCharCode(rnd(25) + 65);
        client.firstName = 'israel ' + letter3 + letter;
        client.lastName = 'Mr. weiss ' + letter2 + letter + letter3;
